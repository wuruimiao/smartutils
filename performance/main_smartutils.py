...
# 包装中间件调用
original_middleware_dispatch = StarletteMiddleware.dispatch
...
StarletteMiddleware.dispatch = middleware_dispatch
...
async def init_app(app: FastAPI):
    ...
    async def get_stats():
        perf_stats.report()
        return {"stats": {k: {"avg": statistics.mean(v), "median": statistics.median(v), "p95": statistics.quantiles(v, n=20)[18], "samples": len(v)} 
