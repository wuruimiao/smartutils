...
def test_url_module():
    ...
    assert not url_mod.is_valid_url(123)  # type: ignore
...
def test_url_edge_cases():
    ...
    assert not url_mod.is_valid_url(None)  # type: ignore
    assert not url_mod.is_valid_url("/path")
    # find_url_in_text
    assert url_mod.find_url_in_text("no url here") is None
    assert url_mod.find_url_in_text("http://a.com and http://b.com").startswith(  # type: ignore
        "http://"
    )
    # 其他边界
