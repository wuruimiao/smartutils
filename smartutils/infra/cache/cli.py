...
class AsyncRedisCli(AbstractResource):
    ...
    async def zrange(self, zset_name: str, start: int, end: int, withscores: bool = False) -> list:
        """
        获取有序集合指定区间成员。
    ...
    async def zrangebyscore(self, zset_name: str, score_min: int, score_max: int, withscores: bool = False) -> list:
        """
        按分数区间获取有序集合成员。
        :return: 成员列表，如 withscores=True 则为[(member, score), ...]
        """
        return await self._redis.zrangebyscore(zset_name, min=score_min, max=score_max, withscores=withscores)
    ...
    # 队列: list
    @asynccontextmanager
    async def safe_rpop_zadd(self, list_ready: str, zset_pending: str, score: int = None) -> AsyncGenerator[
        Optional[str]]:
        """
        安全地从队列弹出任务并放入有序集合。
    ...
    async def safe_rpush_zrem(self, list_ready: str, zset_pending: str, value: str) -> str:
        """
        将某任务从有序集合移除并放回队列。
    ...
    # 队列：zset
    @asynccontextmanager
    async def safe_zpop_zadd(self, zset_ready: str, zset_pending: str, score: int = None) -> AsyncGenerator[
        Optional[str]]:
        """
    ...
    async def safe_zrem_zadd(self, zset_processing: str, zset_ready: str, value: str, score: int) -> str:
        """
    ...
    @asynccontextmanager
    async def xread_xack(self, stream: str, group: str, count: int = 1) -> AsyncGenerator[Optional[dict], None]:
        ...
        try:
            ...
            for message in messages:
                ...
                for message_id, fields in messages_list:
                    ...
                    fields = {
                        key.decode()
                        if isinstance(key, bytes) else key: value.decode()
